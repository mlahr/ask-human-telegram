...
printf 'ask-human-telegram installer %s\n' "$INSTALLER_VERSION"
...
case "$arch" in
	amd64 | arm64)
		...
		;;
	*)
		die "unsupported Debian architecture: $arch; current ask-human-telegram releases publish linux amd64 and arm64 .deb assets only"
		;;
esac
...
asset_name="$(
	awk -v pattern="ask-human-telegram_.*_linux_${asset_arch}[.]deb$" '$2 ~ pattern { print $2 }' "$checksums_file"
)"
